...
-- Setup UI elements
--------------------------------
local mmicon = CreateFrame("Button", "MessageRadarMMIcon", Minimap);
...
mmicon.icon:SetTexture([[Interface\AddOns\MessageRadar\img\logo]]);
...
function _addon:MinimapButtonUpdate()
    if MessageRadar_settings.isActive then
        ...
        mmicon.icon:SetTexture([[Interface\AddOns\MessageRadar\img\logoo]]);
    end
    if MessageRadar_settings.showMinimapButton then
        mmicon:Show();
    else
        mmicon:Hide();
    end
