...
function settings:Setup(savedVarTable, defaultSettings, customName, logoPath, lw, lh, la, lx, ly)
    ...
    if Settings and Settings.RegisterCanvasLayoutCategory then
        ...
        _G.MR_Category = category;  -- 使用更简单的全局变量名
        Settings.RegisterAddOnCategory(category)
        -- 同时保存在设置对象中
        self.categoryID = category
    elseif InterfaceOptions_AddCategory then
        self.name = _addonName;
...
local function CheckBoxOnClick(self)	
    ...
    -- 直接保存设置，不等待确认按钮
    svTable[self.settingName] = self:GetChecked();
...
-- @param row Row to insert it into, if nit row is created (optional)
-- @param width The button width (optional)
-- @return The button frame object
function settings:MakeButton(text, func, row, width)
    local row = row or self:MakeSettingsRow();

	local button = CreateFrame ("Button", nil, row, "UIPanelButtonTemplate");
    button:SetText(text);
	button:SetWidth(width or button:GetTextWidth() + 15);
...
local function DropdownRefresh(self)
    local optionName = "点击查看选项";  -- 改为更友好的提示文本
    local items = self.GetListItems and self.GetListItems() or {};
    
    -- 确保当前设置值在列表中
    local currentValue = svTable[self.settingName];
    if currentValue and items and not items[currentValue] then
        -- 如果是聊天框设置，尝试获取聊天框名称
        if self.settingName == "chatFrame" then
            -- 直接使用聊天框编号作为显示名称，避免使用可能为NOTSET!的原始名称
            items[currentValue] = "聊天窗口 " .. currentValue;
        end
    end
    
    -- 查找当前值对应的显示名称
    if items and currentValue then
...
local function DropdownOpen(self, level, menuList)
    ...
    info.func = function(selfb, arg1, arg2)
        ...
        -- 修改为提示功能，仅对聊天窗口设置显示提示
        if self.settingName == "chatFrame" then
            print("|cFF00FF00MessageRadar|r: 通知发送窗口已修改为: " .. arg2);
        end
        
        -- 直接保存设置，不等待确认按钮
        svTable[self.settingName] = arg1;
    ...
    -- 确保当前设置值在列表中
    local currentValue = svTable[self.settingName];
    if currentValue and items and not items[currentValue] and self.settingName == "chatFrame" then
        -- 直接使用聊天框编号作为显示名称，避免使用可能为NOTSET!的原始名称
        items[currentValue] = "聊天窗口 " .. currentValue;
    end
...
function settings:MakeDropdown(settingName, labelText, tooltipText, width, func, labelWidth, row)
    ...
    -- 创建唯一的下拉菜单名称，避免冲突
    local dropdownName = "MRDropDown_" .. settingName .. "_" .. math.random(1000, 9999);
